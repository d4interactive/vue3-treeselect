...
export default {
  ...
  props: {
    ...
    /**
     * When an ancestor node is selected/deselected,
     * whether its disabled descendants should be selected/deselected.
    ...
    /**
     * Automatically load root options on mount. When set to `false`,
    ...
    /**
     * When user deselects a node, automatically deselect its descendants.
     * Applies to flat mode only.
     */
     ...
     * For single-select mode, it **always** clears the input after selecting
     *  an option regardless of the prop value.
     */
    clearOnSelect: {
      type: Boolean,
      default: false,
     ...
     * starts searching. Used for async search mode.
     ...
     *   - Whenever a branch node has all children checked,
     * the branch node itself will be checked too
    ...
    */
     ...
     * @type {function({action: string, callback: (function((Error|string)=): void),
     *  parentNode: node=, instanceId}): void}
     */
    loadOptions: {
      type: Function,
    },
    ...
    /**
     * Used in conjunction with `showCount`
     *  to specify which type of count number should be displayed.
     * Acceptable values:
     *   - "ALL_CHILDREN"
     *   - "ALL_DESCENDANTS"
     *   - "LEAF_CHILDREN"
     ...
     * Should be `id` or `node` object for single-select mode,
     *  or an array of `id` or `node` object for multi-select mode.
     ...
     *   - "BRANCH_PRIORITY" (default) - If a branch node is
     * checked, all its descendants will be excluded in the `value` array
     *   - "LEAF_PRIORITY" - If a branch node is checked,
     * this node itself and its branch descendants will be
     *  excluded from the `value` array but its leaf descendants will be included
     *   - "ALL_WITH_INDETERMINATE" - Any node that is checked
     *  will be included in the `value` array, plus indeterminate nodes
     */
    valueConsistsOf: {
      type: String,
     ...
     * Note that, when set to `"object"`, only `id` & `label`
     *  properties are required in each `node` object in `value` prop.
     * Acceptable values:
     *   - "id"
     *   - "object"
     */
  ...
  methods: {
    ...
    handleMouseDown: onLeftClick(function handleMouseDown(evt) {
      ...
      if (isClickedOnValueContainer && !this.menu.isOpen
    ...
    handleLocalSearch() {
      ...
      this.traverseAllNodesDFS((node) => {
        if (this.searchNested && splitSearchQuery.length > 1) {
          // eslint-disable-next-line no-param-reassign
          node.isMatched = splitSearchQuery
            .every((filterValue) => match(false, filterValue, node.nestedSearchLabel));
        } else {
          // eslint-disable-next-line no-param-reassign
          node.isMatched = this.matchKeys
            .some((matchKey) => match(!this.disableFuzzyMatching,
              lowerCasedSearchQuery, node.lowerCased[matchKey]));
        }

        if (node.isMatched) {
          this.localSearch.noResults = false;
          // eslint-disable-next-line no-plusplus
          node.ancestors.forEach((ancestor) => this
            .localSearch.countMap[ancestor.id][ALL_DESCENDANTS]++);
          if (node.isLeaf) {
            node
              // eslint-disable-next-line no-plusplus
              .ancestors.forEach((ancestor) => this
    ...
    normalize(parentNode, nodes, prevNodeMap) {
      let normalizedOptions = nodes
        ...
        .map(([node, raw], index) => {
          ...
          const {
            id, label, children, isDefaultExpanded,
          } = node;
          const isRootNode = parentNode === NO_PARENT_NODE;
          const level = isRootNode ? 0 : parentNode.level + 1;
          const isBranch = Array.isArray(children) || children === null;
          const isLeaf = !isBranch;
          const isDisabled = !!node.isDisabled || (!this
            .flat && !isRootNode && parentNode.isDisabled);
          ...
          // this.$ set(normalized, 'ancestors', isRootNode ?
          ...
          if (isLeaf) {
            normalized.ancestors
              // eslint-disable-next-line no-plusplus, no-param-reassign
              .forEach((ancestor) => ancestor.count[LEAF_DESCENDANTS]++);
          }
          if (!isRootNode) {
            // eslint-disable-next-line no-param-reassign
    ...
    loadChildrenOptions(parentNode) {
      ...
      this.callLoadOptionsProp({
        ...
        isPending: () => this.getNode(id).childrenStates.isLoading,
        start: () => {
    ...
    callLoadOptionsProp({
      action, args, isPending, start, succeed, fail, end,
    }) {
    ...
    clear() {
      if (this.hasValue) {
        if (this.single || this.allowClearingDisabled) {
          ...
          this.forest.selectedNodeIds = this.forest
            .selectedNodeIds.filter((nodeId) => this.getNode(nodeId).isDisabled);
        }
